...
namespace Kek
{
    class Task5
    {
        ...
        public Task5()
        {
            ...
            double p1 = Math.Round(this.p, 4);
            if (tackType)
                template = $"Имеются {n} базы с независимым снабжением. Вероятность отсутствия на базе нужного товара равна {p1}. Предприниматель решил закупить некий товар. Составить закон распределения числа баз, на которых в данный момент этот товар отсутствует. Вычислить математическое ожидание и дисперсию этой случайной величины. ";
            else
                template = $"Студент купил {n} билета новогодней лотереи. Вероятность выигрыша по одному билету равна {p1}. Составить закон распределения, вычислить математическое ожидание и дисперсию числа выигрышей среди купленных билетов.";
            template = template;
            this.arrX = arrX;
            this.arrP = arrP;
        }
